...
class Xls extends CFormModel{
    ...
    public function exportPhones($data){
		...
		include($phpExcelPath . DIRECTORY_SEPARATOR . 'chunkReadFilter.php');
			...
			$PHPExcel = PHPExcel_IOFactory::load($tpl.'/phones_tpl.xls');
	...
	$PHPExcel->getActiveSheet()->setCellValue("A$i",$row['fio']); 
	$PHPExcel->getActiveSheet()->setCellValue("B$i",$row['post']); 
	...
	$PHPExcel->getActiveSheet()->setCellValue("L$i",$row->idWorkplace->department());
	$PHPExcel->getActiveSheet()->setCellValue("M$i",$row->mac);
	$PHPExcel->getActiveSheet()->setCellValue("N$i",$row->hostname);
	/*
	...
	$PHPExcel->getActiveSheet()->setCellValue("O$i",$row->measures_taken); */
			...
			$PHPExcel->getActiveSheet()->getStyle('A3:'.'N'.$i)->applyFromArray($bordercells);
			$name='v';
