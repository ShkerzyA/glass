<?php
         ...
         $producer=(isset($data->producer))?$data->producer0->name:'';
   		echo'<tr><td>SN: <b>'.$data->serial.'</b></td><td rowspan=2>'.$data->type0->name.'/ '.$producer.'/ '.$data->mark.'</td><td rowspan=2>'.$data->idWorkplace->wpNameFull().'</td><td>'.$status[$data->status].'</td><td rowspan=2>'.$data->notes.'</td>'.$edit.'</tr>';
