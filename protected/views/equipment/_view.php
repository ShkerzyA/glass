<?php
         ...
         $producer=(isset($data->producer))?$data->producer0->name:'';
   		echo'<tr class="str_eq"><td>'.$data->serial.'</td><td>'.$data->type0->name.'/ '.$producer.'/ '.$data->mark.'</td><td>'.$data->inv.'</td><td>'.$status[$data->status].'</td><td>'.$data->notes.''.($pid=(!empty($data->parent_id))?'(pid '.$data->parent_id.')':'').'</td>'.$edit.'</tr>';
